...
| DEF-004 | CSV import replaces all data without confirmation                                                                                                                                                                                                                          | [1.12](p1-12-coverage-csv.md) — `csv-import-safety.spec.ts`                                              | [3.4b](p3-04b-csv-clipboard-defects.md)                                                                                                                                                                                                                                              | closed                 |
...
| DEF-006 | A stray _balanced_ quote destroys data on import — `"a"b"c"` is accepted silently and replaces every group with one named `abc`                                                                                                                                            | [1.12](p1-12-coverage-csv.md) — `csv-import-safety.spec.ts`                                              | [3.4b](p3-04b-csv-clipboard-defects.md)                                                                                                                                                                                                                                              | closed                 |
| DEF-007 | CSV export has no UTF-8 BOM; Cyrillic breaks in Excel                                                                                                                                                                                                                      | [1.12](p1-12-coverage-csv.md) — `csv-export-contract.spec.ts`                                            | [3.4b](p3-04b-csv-clipboard-defects.md)                                                                                                                                                                                                                                              | closed                 |
...
| DEF-013 | "Clear all data" leaves the template key behind                                                                                                                                                                                                                            | [1.4](p1-04-feature-specs-1.md) — `data-reset.spec.ts`, `fixme`                                          | [3.4b](p3-04b-csv-clipboard-defects.md)                                                                                                                                                                                                                                              | closed                 |
...
| DEF-015 | Real personal payment identifiers ship in the default template (lines 387–392, 400)                                                                                                                                                                                        | not a spec — a grep gate                                                                                 | [2a.4](p2a-04-cutover.md) — the file holding the values is deleted; [3.5](p3-05-pii-template-cleanup.md) added the build check that keeps them out, and git history stays the owner's separate decision                                                                              | closed (working tree)  |
...
| DEF-017 | Inline month price input is rendered into `#monthlySection` by the same handler that sets that section to `display: none`, so no user can ever see or reach it                                                                                                             | [1.3](p1-03-scaffold-core.md) (attachment only)                                                          | [3.7](p3-07-cleanup.md) — the dead branch is deleted (owner decision, 2026-09-11): the calendar's bulk price already sets a month price, and it is the control the user is looking at while choosing that month's dates. `month-price-input` left the frozen testid contract with it | closed                 |
...
| DEF-025 | Every control's border is `#ccc`: **1.61:1** on a panel and **1.53:1** on the page, where WCAG 2.2 AA 1.4.11 asks 3:1 of a boundary that carries meaning. It is the outline that says where a button ends                                                                  | [2b.7](p2b-07-styles-extraction.md) — measured, and deferred on the record rather than omitted           | [3.6](p3-06-a11y-verification.md) — `--border-control` is `#868f9e`: 3.26:1 on a panel and 3.12:1 on the page, asserted from the stylesheet by `contrast.test.ts`                                                                                                                    | closed                 |
| DEF-027 | The document is `lang="en"` and the teacher's data is Ukrainian, with no `lang` on it, so a screen reader pronounces her group names and her payment message with English phonetics — WCAG 2.2 AA 3.1.2                                                                    | [3.6](p3-06-a11y-verification.md) — found by the manual checklist, not by a scan                         | Deferred to Phases 4–6 with an owner decision: a language setting, per-group, or detection. Not a Phase 3 fix — it is about data, not chrome                                                                                                                                         | open (decision needed) |
