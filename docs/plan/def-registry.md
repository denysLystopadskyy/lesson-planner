...
| DEF-016 | Modal open/close fade no longer plays; `hidden` is set in the same tick as the `show` class, so the `opacity 0.2s` transition is dead code | [1.3](p1-03-scaffold-core.md)                                             | [3.4a](p3-04a-interaction-defects.md)                               | open                   |
